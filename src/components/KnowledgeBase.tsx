...
const KnowledgeBase = () => {
  ...
  const supportedFormats = [
    { name: t('knowledgeBase.supportedFormats.formats.pdf.name'), icon: FileText, color: 'text-red-500', count: t('knowledgeBase.supportedFormats.formats.pdf.count') },
    { name: t('knowledgeBase.supportedFormats.formats.excel.name'), icon: FileSpreadsheet, color: 'text-green-500', count: t('knowledgeBase.supportedFormats.formats.excel.count') },
    { name: t('knowledgeBase.supportedFormats.formats.word.name'), icon: FileText, color: 'text-blue-500', count: t('knowledgeBase.supportedFormats.formats.word.count') },
    { name: t('knowledgeBase.supportedFormats.formats.images.name'), icon: Image, color: 'text-purple-500', count: t('knowledgeBase.supportedFormats.formats.images.count') },
    { name: t('knowledgeBase.supportedFormats.formats.websites.name'), icon: Globe, color: 'text-teal-500', count: t('knowledgeBase.supportedFormats.formats.websites.count') },
    { name: t('knowledgeBase.supportedFormats.formats.videos.name'), icon: Video, color: 'text-orange-500', count: t('knowledgeBase.supportedFormats.formats.videos.count') },
  ];
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">
              {t('knowledgeBase.supportedFormats.title')}
            </h3>
            ...
            <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-xl p-6 border border-teal-100">
              <h4 className="font-semibold text-gray-900 mb-3">{t('knowledgeBase.supportedFormats.richResponse.title')}</h4>
              <div className="grid grid-cols-2 gap-3 text-sm text-gray-700">
                {t('knowledgeBase.supportedFormats.richResponse.features', { returnObjects: true }).map((feature, index) => (
            ...
            >
              ...
              <span>{t('knowledgeBase.supportedFormats.addDocuments')}</span>
            </button>
          </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-8">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-teal-500 to-blue-500 p-4 text-white">
                ...
                <p className="text-blue-100 text-sm">{t('knowledgeBase.subtitle')}</p>
              </div>
              ...
              <div className="p-6">
                ...
                {uploadedFiles.length > 0 && (
                  <div className="space-y-2 mb-6">
                    <h4 className="font-semibold text-gray-900">{t('knowledgeBase.uploadModal.title')}:</h4>
                ...
                <div className="space-y-3">
                  <h4 className="font-semibold text-gray-900">{t('blog.latest')}:</h4>
                  <div className="space-y-2">
                    {['Product_Manual.pdf', 'FAQ_Database.xlsx', 'Company_Policies.docx'].map((file, index) => (
                      <div key={index} className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
                        ...
                        <div className="text-xs text-green-600 font-medium">{t('common.search')}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-100 p-6 flex justify-between items-center">
                <h3 className="text-2xl font-bold text-gray-900">{t('knowledgeBase.uploadModal.title')}</h3>
                <button
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {t('knowledgeBase.uploadModal.demoFiles', { returnObjects: true }).map((file, index) => (
                    <button
                      key={index}
                ...
                >
                  {t('knowledgeBase.uploadModal.close')}
                </button>
              </div>
            </div>
          </div>
